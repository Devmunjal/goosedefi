...
const CakeStats = () => {
  ...
  if(farms && farms[0] && farms[0].enterPerBlock){
    enterPerBlock = new BigNumber(farms[0].enterPerBlock).div(new BigNumber(10).pow(18)).toNumber();
  }
    ...
    <StyledCakeStats>
      <CardBody>
        ...
        <Row>
          ...
          {cakeSupply && <CardValue2 fontSize="14px" value={cakeSupply} decimals={0} />}
        </Row>
        <Row>
          <Text fontSize="14px">{TranslateString(540, 'New EGG/block')}</Text>
          <Text bold fontSize="14px">{enterPerBlock}</Text>
        </Row>
      </CardBody>
    </StyledCakeStats>
  )
